...
class PluginUpdateChecker_1_3 {
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile, $pluginData = null, $status = null) {
		if ( $pluginFile == $this->pluginFile && current_user_can('update_plugins') ) {
			...
			$linkText = apply_filters('puc_manual_check_link-' . $this->slug, 'بررسی به روز رسانی');
			if ( !empty($linkText) ) {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				$message = 'شما از آخرین نسخه افزونه استفاده می کنید.';
			} else if ( $status == 'update_available' ) {
				$message = 'نسخه جدیدتری از افزونه در دسترس است.';
			} else {
				$message = sprintf('وضعیت ناشناخته ای برای بررسی آپدیت: "%s"', htmlentities($status));
			}
			printf(
...
class PluginInfo_1_3 {
	...
	public static function fromJson($json, $triggerErrors = false){
		...
		if ( !$valid ){
			if ( $triggerErrors ) {
				trigger_error(
					"فایل .json افزونه قابل استفاده نیست!",
					E_USER_NOTICE
				);
			}
			return null;
